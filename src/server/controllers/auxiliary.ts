...
module.exports.ClaimAppointment = function (req: api.Request & swaggerTools.Swagger20Request<ClaimAppointmentPayload>, res: express.Response) {
    ...
    db.schedules.findOne(req.swagger.params.schedules.value.scheduleID).then((schedule) => {

        if (schedule){
            db.reservations.find(req.swagger.params.reservations.scheduleinfo.value.scheduleID && req.swagger.params.reservations.scheduleinfo.value.date && req.swagger.params.reservations.scheduleinfo.value.time).toArray().then((appointments) => {
                if (appointments) {
